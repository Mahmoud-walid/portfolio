...
const routerDescriptions: Record<string, string> = {
...
interface VercelInfoProps {
  info: {
    framework?: string;
    nodeVersion?: string;
    details?: {
      repositoryFrameworks?: Framework[];
      nextjsVersion?: string;
      astroVersion?: string;
      nextjsLatestVersion?: string;
      routerMode?: string;
...
export function VercelInfo({ info }: VercelInfoProps) {
  ...
  const routerDescription =
    routerDescriptions[details.routerMode || 'none'] || routerDescriptions.none;
      ...
      <Popover
        ...
        content={
          <>
            ...
            <span>
              {framework.name}: {framework.version} → {framework.latestVersion}
            </span>
          </>
        }
      />
  ...
  const frameworkIcon =
    info.framework === 'nextjs' ? (
      <Popover
        button={
          <FrameworkImage src="/nextjs-icon-dark.svg" alt="Next.js icon" />
        }
      ...
      <Popover
        ...
        content={`Astro project${astroVersion ? ` (v${astroVersion})` : ''}`}
      />
    ) : null;
...
function renderFrameworkIcon(framework: Framework, index: number) {
...
function renderUiLibrary(library: string) {
  const libraries: Record<string, { icon: React.ReactNode; label: string }> = {
    tailwindcss: {
  ...
  return <Popover key={library} button={config.icon} content={config.label} />;
}

function FrameworkImage({ src, alt }: { src: string; alt: string }) {
  return (
    <Image
      height={16}
      width={16}
