...
export function ProjectTechnology({
  ...
  const { status, repository, showLoader } =
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <span className="sr-only">Loading repository technology details</span>
      ) : status === 'error' ? (
...
function getAlertPresentation(alerts: any) {
  if (!alerts) {
    return {
      color: undefined,
      count: null,
...
export function ProjectMetrics({ repositoryKey }: { repositoryKey: string }) {
  const { status, repository, showLoader } =
    useProjectSecondaryData(repositoryKey);
  const isLoading = status === 'loading';
    ...
    >
      {isPending ? (
        ...
        <>
          ...
          >
            <GoDependabot className="size-4 shrink-0" fill={alerts.color} />
          ...
          <AgentMetric
            ...
            icon={<SiGithubcopilot className="size-3.5 text-[#8534F3]" />}
          />
          <AgentMetric
...
function AgentMetric({ count, href, icon, label }: AgentMetricProps) {
