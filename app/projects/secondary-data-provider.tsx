...
interface SecondaryDataState {
  status: 'restoring' | 'loading' | 'ready' | 'error' | 'disabled';
  repositories: Record<string, any>;
  generatedAt: string | null;
  showLoader: boolean;
}

const SecondaryDataContext = createContext<SecondaryDataState | null>(null);
...
function isUsableSnapshot(snapshot: any) {
  return Boolean(
...
function storeClientSnapshot(payload: any) {
  const snapshot = {
...
function createReadyState(snapshot: any): SecondaryDataState {
  return {
    status: 'ready',
...
export function SecondaryDataProvider({ children }: { children: ReactNode }) {
  const [state, setState] = useState<SecondaryDataState>({
    status: 'restoring',
    repositories: {},
    generatedAt: null,
    showLoader: false,
...
export function useProjectSecondaryData(repositoryKey: string) {
